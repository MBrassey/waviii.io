...
class Wallet extends React.Component {
  ...
  render() {
    ...
    if(this.state.loading) {
      if(this.state.noEth) {
        content = <div className="content mr-auto ml-auto" style={{ width: "90%" }}><div className="card mb-4"><div className="card-body" style={{ width: "90%" }}><p id="loader" ><WaveTopBottomLoading color={"#2c91c7"} /><a href="https://metamask.io/"><h5><strong>Install MetaMask!</strong></h5><img alt="MetaMask" width="100%" height="auto" src={require("assets/img/mm.png")} /></a></p></div></div></div>
      } else {
              ...
              { this.state.transactions.sort((a, b) => a.blockNumber < b.blockNumber).map((tx, key) => {
                return (
                  <tr key={key} >
                    ...
                    >
                      <span className="text-muted">{tx.returnValues.to}</span>  
                  </a></td>
                  {tx.returnValues.to !== this.state.account &&
                    <td><span className="waviii">-<CountUp duration={2.7} start={0} separator="" decimals="2" decimal="." end={window.web3.utils.fromWei(tx.returnValues.value.toString(), 'Ether')} /></span></td>
                   }
                  {tx.returnValues.to === this.state.account &&
                    <td><span className="waviii2">+<CountUp duration={2.7} start={0} separator="" decimals="2" decimal="." end={window.web3.utils.fromWei(tx.returnValues.value.toString(), 'Ether')} /></span></td>
                   }
